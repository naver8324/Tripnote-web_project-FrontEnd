...
const FindPassword = ({ email, setEmail, isVerified, setIsVerified }) => {
    ...
    const { findPassword, loading:findPasswordLoading, error: findPasswordError } = useFindPassword();
        ...
        try {
            ...
            if (result) {
                ...
                try{
                    ...
                    if(result){
                        ToastAlert(`새로운 비밀번호가 ${email} 로 전달되었습니다.`, 'success');
                    }
                }catch(err){
                    console.error('Error reset password:', err);
                    setEmailError('새로운 비밀번호를 이메일로 전달하는 도중 오류가 발생했습니다.');
                    setEmailErrorColor('red-500');
                }
